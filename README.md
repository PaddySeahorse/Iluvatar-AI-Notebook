...
- **ReAct Agent 助手** — AI Chat 升级为轻量 ReAct 代理：可调用工具执行内核代码单元、查询变量表与 GPU 状态、列出/读取 Notebook 文件，再基于工具结果作答；自动探测 LLM 是否支持 function calling，不支持时降级为文本 JSON 协议；Agent 具备专业的 IXUCA（天数智芯软件栈）知识，可解答芯片工具链与算子适配问题
