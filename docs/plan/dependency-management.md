...
> **说明**：实际 `requirements.txt` 已裁剪掉 `pynvml`（GPU 遥测通过 IXUCA SDK 实现）、`pylint`（使用 stdlib `ast` 做静态分析）、`python-dotenv`（`app.py` 手动解析 `.env`）、`pandas`/`numpy`（非项目依赖，仅在用户代码中使用）。
